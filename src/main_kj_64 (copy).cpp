...
public:
        ...
        for (size_t j = 0; j < Horizon_SCAN; ++j)
        {
            for (size_t i = 0; i < N_SCAN; ++i)
            {
                    ...
                    //fullCloud->points[upperInd].intensity == -1)
                {
                    ...
                    // fullCloud->points[lowerInd].intensity = 1;
                    // fullCloud->points[lowerInd].x = 30;
                    // fullCloud->points[lowerInd].y = 1;
                    // fullCloud->points[lowerInd].z = -1;
                    // fullCloud->push_back(fullCloud->points[lowerInd]);
                    groundMat.at<int8_t>(i, j) = -1;
                    // labelMat.at<int8_t>(i, j) = -1;
                    continue;
                }
                if (i <= groundScanInd)
                {   
                    upperInd = j + (i + 1) * Horizon_SCAN;
                    // if (fullCloud->points[upperInd].intensity == -1)
                    // {
                    //     upperInd = j + (i + 2) * Horizon_SCAN;
                    // }
                    // else if ((fullCloud->points[upperInd + Horizon_SCAN].intensity == -1))
                    //     upperInd = j + (i + 1) * Horizon_SCAN;
                    ...
                    // printf(typeid(angle).name());

                    // if (isnan(fullCloud->points[upperInd].x))
                    //     printf("%f deg, x : %f , y: %f, z: %f \n",(angle, diffX, diffY, diffZ));
                    // angle이 theshold를 넘지 못하면 ground로 판단.
                    // if (abs(diffZ)<0.10)
                    if (abs(angle - sensorMountAngle) <= groundThreshold) //|| fullCloud->points[lowerInd].x < 10)
                    //(isnan(angle) && fullCloud->points[lowerInd].z < -1))
                    //(fullCloud->points[lowerInd].x > -1.5 && fullCloud->points[lowerInd].x < 1.5 && fullCloud->points[lowerInd].y > -1 && fullCloud->points[lowerInd].y < 1))
                    {
                        groundCloud->push_back(fullCloud->points[lowerInd]);
                        groundCloud->push_back(fullCloud->points[upperInd]);
                        groundMat.at<int8_t>(i, j) = 1;
                        groundMat.at<int8_t>(i + 1, j) = 1;
                        // groundCloud->push_back(fullCloud->points[j + i * Horizon_SCAN]);
                        // labelMat.at<int8_t>(i, j) = 1;
                        // labelMat.at<int8_t>(i + 1, j) = 1;
                    }
                    // else 
                    // {
                    ...
                    //     angle = atan2(diffZ, sqrt(diffX * diffX + diffY * diffY)) * 180 / M_PI;
                    //     // if (abs(diffZ)<0.10)
                    //     if (abs(angle - sensorMountAngle) <= groundThreshold)
                    //     {
                    //         groundCloud->push_back(fullCloud->points[lowerInd]);
                    //         groundCloud->push_back(fullCloud->points[upperInd]);
                    //         groundMat.at<int8_t>(i, j) = 1;
                    ...
                    //     }
                    if (fullCloud->points[lowerInd].x < MAX_X && fullCloud->points[lowerInd].x > MIN_X && \
                        fullCloud->points[lowerInd].y < MAX_Y && fullCloud->points[lowerInd].y > MIN_Y && \
                        fullCloud->points[lowerInd].z < 0 && groundMat.at<int8_t>(i, j) == 0)
                    {
                ...
                //     fullCloud->points[lowerInd].x < MAX_X && fullCloud->points[lowerInd].x > MIN_X &&
                //     fullCloud->points[lowerInd].y < MAX_Y && fullCloud->points[lowerInd].y > MIN_Y)
                //     groundRemovedCloud->push_back(fullCloud->points[j + i * Horizon_SCAN]);
            }
        }
    }
    ...
    {
        // Clustering
        vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> cloudClusters = pointProcessor.Clustering(groundRemovedCloud, 1.0, 3, 1000);
        // Processing af
        int clusterId = 0;
